...
class LiteWallet {
  ...
  init() {
    return new Promise(async (resolve, reject) => {
      ...
      this.lastSyncId = JSON.parse(
        await native.zingolib_execute_async("syncstatus", "")
      ).sync_id;

      const syncPoller = setInterval(async () => {
        var syncStatus = JSON.parse(
          await native.zingolib_execute_async("syncstatus", "")
        );
  ...
  async createNewAddress(type) {
    ...
    const addrDetail = await this.fetchAllAddresses().filter(
      (addr) => addr.address === ua[0]
    );
  ...
  async fetchTandZTransactions(latestBlockHeight) {
    ...
    let txList = list.map((tx) => {
      ...
      transaction.address =
        type === "sent"
          ? tx.outgoing_metadata.length > 0
            ? tx.outgoing_metadata[0].address
            : ""
          : tx.address;
      transaction.type = type;
      transaction.amount = tx.amount / 10 ** 8;
      transaction.confirmations = tx.unconfirmed
        ? 0
        : latestBlockHeight - tx.block_height + 1;
    ...
    txList = txList.filter(
      (tx) =>
        !(tx.type === "sent" && tx.amount < 0 && tx.detailedTxns.length === 0)
    );
    ...
    m.forEach((txns) => {
      ...
      combinedTx.detailedTxns = this.combineTxDetails(
        txns.flatMap((tx) => tx.detailedTxns)
      );
  ...
  async refresh(fullRefresh) {
    ...
    if (
      ...
      console.log(
        "Refreshing wallet: " +
          (latestBlockHeight - this.lastBlockHeight) +
          " new blocks."
      );
  ...
  async fetchTotalBalance() {
    ...
    balance.verified_sapling_balance =
      balanceJSON.verified_sapling_balance / 10 ** 8;
    balance.spendable_sapling_balance =
      balanceJSON.spendable_sapling_balance / 10 ** 8;
    balance.unverified_sapling_balance =
      balanceJSON.unverified_sapling_balance / 10 ** 8;

    // Every orchard balance
    balance.orchard_balance = balanceJSON.orchard_balance / 10 ** 8;
    balance.verified_orchard_balance =
      balanceJSON.verified_orchard_balance / 10 ** 8;
    balance.spendable_orchard_balance =
      balanceJSON.spendable_orchard_balance / 10 ** 8;
    balance.unverified_orchard_balance =
      balanceJSON.unverified_orchard_balance / 10 ** 8;
  ...
  async fetchUnspentNotes() {
    ...
    unspentJSON.unspent_sapling_notes.forEach((s) => {
      ...
      addressBalances.sapling.push({
        address: z_address[0].receivers.sapling,
        value: s.value,
      });
    });
    ...
    unspentJSON.utxos.forEach((s) => {
      ...
      addressBalances.transparent.push({
        address: t_address[0].receivers.transparent,
        value: s.value,
      });
    });
  ...
  async fetchPendingNotes() {
    ...
    pendingJSON.pending_orchard_notes.forEach((s) => {
      pendingAddressBalances.orchard.push({
        address: s.address,
        value: s.value,
      });
    });
    ...
    pendingJSON.pending_sapling_notes.forEach((s) => {
      let z_address = addresses.filter((addr) => s.address === addr.address);
      pendingAddressBalances.sapling.push({
        address: z_address[0].receivers.sapling,
        value: s.value,
      });
    });
    ...
    pendingJSON.pending_utxos.forEach((s) => {
      let t_address = addresses.filter((addr) => s.address === addr.address);
      pendingAddressBalances.transparent.push({
        address: t_address[0].receivers.transparent,
        value: s.value,
      });
    });
  ...
  async fetchAllAddresses() {
    const addressesJSON = JSON.parse(
      await native.zingolib_execute_async("addresses", "")
    );
  ...
  async fetchAddressesWithBalance() {
    ...
    const oaddresses = balanceJSON.ua_addresses.map((o) => {
    ...
    const zaddresses = balanceJSON.z_addresses.map((o) => {
      // If this has any unconfirmed txns, show that in the UI
      const ab = new AddressBalance(o.address, o.balance / 10 ** 8);
    ...
    const taddresses = balanceJSON.t_addresses.map((o) => {
      // If this has any unconfirmed txns, show that in the UI
      const ab = new AddressBalance(o.address, o.balance / 10 ** 8);
  ...
  async getDefaultFee() {
    const fee = JSON.parse(
      await native.zingolib_execute_async("defaultfee", "")
    );
  ...
  async getSendProgress() {
    const res = JSON.parse(
      await native.zingolib_execute_async("sendprogress", "")
    );
    return res;
  }
  ...
  async fetchWalletSettings() {
    ...
    try {
      const spam_filter_str = await native.zingolib_execute_async(
        "getoption",
  ...
  async setWalletSettingOption(name, value) {
    return new Promise(async (resolve, reject) => {
      try {
        const r = await native.zingolib_execute_async(
          "setoption",
          `${name}=${value}`
        );
