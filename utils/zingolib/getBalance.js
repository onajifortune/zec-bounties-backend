...
async function getAddressBalance(options = {}) {
  try {
 ...
 */
async function getSapplingAddressBalance(options = {}) {
  try {
    const result = await getAddressBalance(options);
    console.log("Result", result.sapling_balance);
    return result;
  } catch (error) {
    return false;
  }
...
module.exports = { getAddressBalance, getSapplingAddressBalance };
